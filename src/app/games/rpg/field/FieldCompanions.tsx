...
/** 궤적의 최신 지점부터 뒤로 dist 만큼 걸어간 위치 (궤적이 짧아 dist에 못 미치면 null) */
function pointBehind(crumbs: Crumb[], leader: Crumb, dist: number): Crumb | null {
    let remain = dist;
    let cur = leader;
    ...
    // 궤적이 아직 followDist만큼 쌓이지 않음 — 호출부에서 제자리 대기로 처리
    return null;
}
...
function Companion({
    ...
    useFrame((_, dt) => {
        ...
        if (!placed.current) {
            // 초기 배치: 리더와 완전히 겹치지 않도록 대각선 옆자리에 세운다
            const side = followDist * 0.55;
            g.position.set(leader.x - side, leader.y, leader.z + side);
            renderY.current = leader.y;
            placed.current = true;
            return;
        }

        // 리더가 순간이동(패스트트래블/전투 복귀)하면 옆자리로 워프 (겹침 방지)
        ...
        if (leaderGap > TELEPORT_SNAP) {
            ...
            // 워프한 프레임에도 위치를 바로 공개해 1프레임 지연을 없앤다
            const snapStore = (scene.userData.__companionPositions ??= {}) as Record<
                string,
        ...
        if (target) {
            ...
            // 궤적이 followDist만큼 못 쌓임(초기 배치 직후 등) — 제자리 대기, y는 현재값 유지
            if (animState !== "idle") setAnimState("idle");
        }

        // 화자 마커 등에서 쓸 위치 공유 (대기 프레임에도 매번 갱신)
    ...
    // 동행에서 제외되면(파티원 교체·이탈 등) 남아있는 위치 기록을 정리한다
    useEffect(() => {
        return () => {
            const store = scene.userData.__companionPositions as
                | Record<string, { x: number; y: number; z: number }>
                | undefined;
            if (store) delete store[charId];
        };
    }, [scene, charId]);
