...
export const DUNGEONS: DungeonDef[] = [
    ...
    //
    // ⚠ [2차 리뷰 수정 — 실버그] 지하 게이트 착지점(-184.8,-46.7)과 문1 스위치(당시
    // -183,-46.6)가 1.80m였다 — 둘 다 INTERACT_RANGE 2.6의 E 인터랙터블이고
    // DungeonController(게이트)·DungeonDoor(스위치)의 keydown 리스너가 상호 배제
    // 없이 병존해(DungeonController.tsx:101-125, DungeonDoor.tsx:97-117), 스위치1
    // 앞에서 E를 누르면 문 개방과 지상 귀환(게이트 상행엔 requireFlag 가드가 없음)이
    // 동시 발화하는 실버그였다. 재배치 3차 시도 끝에 (-182.3,-44.0)으로 확정:
    //   1차(-181.6,-45.0, 북쪽): 게이트 3.62m 확보했으나 문1 박스(x -182.1~-180.9)
    //     안쪽이라 문이 닫혀 있으면 스위치 자체에 도달 불가 — 치명적 배치 오류.
    //   2차(-181.3,-47.5, 남쪽): 문1 박스는 벗어났으나 게이트와 3.59m — 오차범위서
    //     통과했지만 재확인 중 같은 x가 여전히 문1 박스 안(-181.3 > -182.1)임을
    //     재발견, 남쪽은 z<-48 부근에서 곧장 다른 층(y=-56)으로 떨어져(disconnected
    //     남쪽 방) 더 밀 여유도 없었다.
    //   3차(확정, -182.3,-44.0, 북서쪽): x=-182.3으로 문1 박스 서쪽면(-182.1)보다
    //     0.2m 여유 있게 벗어나면서, 북쪽으로 게이트와 3.68m(≥3.5m, 기존 던전
    //     전례: port 4.47m·hill 11.84m) 확보. 정예1(branch1)과는 4.84m로
    //     SIGHT_RANGE(7m) 이내라 완전한 구조적 안전은 아니나(§ gameData.ts
    //     FIELD_ENEMIES 주석의 배치 한계 실증 참조 — 이 방에서 게이트·문1 회피와
    //     정예1 회피를 동시에 완전 달성하는 자리는 없음), 원안(1.5~1.8m)보다는
    //     대폭 개선했고 배터리 로머 난입 가드로 흡수됨을 재확인(run11 이후 로그).
    // 배터리에
    // "스위치1 E 시 던전 잔류(지상 귀환 미발화)" 체크를 추가해 재발 방지
    // (§ task-2-report.md 부록 A).
    {
        id: "forest_rootcave",
        label: "뿌리 굴",
...
export const DUNGEON_DOORS: DungeonDoorDef[] = [
    ...
    // (scratchpad/sp2b-t2-verify-direct.js, 재배치 후 좌표 재검증 포함).
    {
        ...
        // [2차 리뷰 수정] 원안(-183,-46.6)은 지하 게이트(-184.8,-46.7)와 1.80m라
        // E 인터랙터블(둘 다 반경 2.6) 상호 배타가 안 돼 스위치1 앞 E가 게이트
        // 상행과 동시 발화하는 실버그였다 — 3차 재배치 끝에 (-182.3,-44.0) 확정
        // (문1 박스 x -182.1~-180.9 바깥 0.2m 여유, 게이트와 3.68m·정예1과 4.84m —
        // 위 DUNGEONS 주석 "2차 리뷰 수정" 절에 3회 시도 전문 기록).
        switch: { pos: { x: -182.3, y: -44.25, z: -44.0 }, label: "뿌리 굴 통로 1" },
    },
    {
        id: "forest_rootcave_door2",
        flag: "door_forest_rootcave_2",
        door: { pos: { x: -173.5, y: -44.25, z: -44.5 }, size: [1.2, 6, 5.0] },
        // [2차 재배치] 원안(-176,-45.5)은 당시 정예2(-177.5,-46.5)와 1.80m로
        // 너무 가까웠다(수치 정정 — 최초 리뷰 반영 커밋의 주석이 이 1.80m와
        // 재배치 후 4.03m를 뒤바꿔 적었던 오기 정정). 문 바로 옆(-174,-44.5, 문2
        // 서쪽면과 0.5m — sp0/T3 관례상 스위치는 문 인근이 정상)으로 당겨 정예2
        // (현재 -178,-46.5)와 4.47m 확보(§ gameData.ts FIELD_ENEMIES 주석).
        //
        // [T3 인터페이스 노트] 이 스위치(-174,-44.5)는 FOREST_ROOTCAVE_BOSS_ENTRY
        // (-170.9,-44.8)와 3.11m로 가깝다 — T3가 보스 트리거를 이 지점에 놓을 때
        // 스위치2 E 판정과 보스 트리거 근접 충돌 가능성을 감안할 것(본 태스크
 ...
 * 동쪽 곁가지. 실텔레포트 착지 확인 dy=0.00(별도 세션 2회 재현,
 * scratchpad/sp2b-t2-verify-direct.js).
 *
 * [정직한 서술 — 재리뷰 반영, 과잉 주장 정정] 정적 스폰 거리는 정예1(-178.5,
 * -45.25,-41) 8.50m·정예2(-178,-44.25,-46.5) 7.30m로 FieldEnemyAvatar.tsx의
 * SIGHT_RANGE=7(25행, CLOSE_DETECT_RANGE=2.5는 26행 — 이전 주석의 27~28행 인용은
 * 오기, 정정)을 넘긴다. 그러나 이것만으로 "구조적으로 감지 불가"라 단정하는 것은
 * 과잉 주장이었다 — 실제로는 ① WANDER_RADIUS=4(:19, r=0.5+rand*4 → 스폰에서 최대
 * ~4.5m까지 배회)로 정적 거리의 안전 마진이 잠식되고 ② 정예2는 스위치2(-174,-44.5)
 * 와 4.47m(SIGHT_RANGE 이내)라 스위치2 조작 중 어그로가 붙으면 CHASE_SPEED=2.8·
 * CHASE_FROM_HOME=15(:28,30, :375-380·:403-424)로 스폰에서 최대 15m까지 추격이
 * 이어질 수 있어 — 정예2가 어그로 상태로 추격 중이라면 이 보스 진입점(스위치2와
 * 3.11m)까지 따라붙는 경로 자체를 정적 거리 비교만으로는 배제할 수 없다.
 *
 * 실제 보장되는 것은 **보스 트리거 발동 순간(컷신/전투 개시 중)의 난입 차단**이다
 * — FieldPlayer.tsx:677-684의 `inDialogue`(:483-490, `!!gState.cutscene` 포함)
 * 게이트가 매 프레임 적 충돌 판정(onEnemyCollide, :686-722) 전체를 조건 없이
 * 스킵시킨다. 헤드리스로 격리 재현(scratchpad/sp2b-t2-cutscene-gate-verify2.js,
 * 세션 분리 — 동일 세션 연속 테스트는 handleEnemyCollide의 500ms 지연
 * startCombat이 다음 테스트로 새어나가는 레이스가 있어 격리 필수, 최초 시도의
 * 실패 원인이었음): 컷신(cs_theo_camp) 활성 상태로 먼저 전환한 뒤 정예2 스폰점에
 * 거리=0으로 텔레포트해 4초 폴링 — cutscene 진실값 내내 유지·combat.phase
 * 전 구간 idle 확인. 즉 T3가 보스 트리거를 cutscene 진입과 함께 발동시키는 한
 * (SP2a 기존 패턴 그대로), 컷신 시작 이후의 난입은 프레임워크 게이트로 실제
 * 차단된다 — 다만 **트리거 발동 이전, 플레이어가 이 지점에 접근하는 구간의
 * 확률적 조우는 배제되지 않는다**(SP2a-T4가 6.71m 사례에서 확립한 전례와 동일
 * 성격 — "컷신 중 난입 불가는 실증하되 접근 구간 조우는 기록 수용"). T3는 보스
 * 트리거 발동 전 정예2와의 조우 가능성을 전제하고 설계할 것(§ task-2-report.md
 * 부록 B).
