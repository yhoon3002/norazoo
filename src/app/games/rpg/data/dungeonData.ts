 ...
 *
 * 정예1(-178.5,-45.25,-41)과 8.50m(≥8m ✓)·정예2(-178,-44.25,-46.5)와 7.30m 이격.
 * 정예2 쪽은 8m 관례 기준 미달이나(§ gameData.ts FIELD_ENEMIES 주석의 배치 한계
 * 실증 참조), FieldEnemyAvatar.tsx의 실제 감지 로직(SIGHT_RANGE=7·
 * CLOSE_DETECT_RANGE=2.5, 27~28행)상 두 정예 모두 이 보스 진입점에서
 * SIGHT_RANGE(7m)를 넘겨(정예1 8.50m·정예2 7.30m) **거리만으로 이미 시야 감지
 * 조건(playerDist > SIGHT_RANGE → false, FieldEnemyAvatar.tsx:356)을 만족하지
 * 못한다** — 방향/시야각(FOV)과 무관하게 구조적으로 감지 불가. 즉 8m 관례엔
 * 못 미쳐도 T3 보스 트리거 발동 시점 기준으로는 정예 난입이 원천 차단된다.
 * (T3 인터페이스 노트: 문2 스위치와는 3.11m로 더 가까우니 스위치 조작 구간은
 * 별도 유의 — 위 forest_rootcave_door2 주석 참조.) */
