...
import { TailorPanel } from "../menu/TailorPanel";
        ...
        const handleKey = (e: KeyboardEvent) => {
            ...
            if (combat.phase === "idle") {
                if (key === "escape") {
                    ...
                    if (ui.shopOpen || ui.fastTravelOpen || ui.mapOpen || ui.fishingOpen || ui.smithOpen || ui.bountyOpen || ui.tailorOpen) {
                        closeAll();
                        return;
                    }
                    togglePause();
                    ...
                    if (u.mapOpen || u.shopOpen || u.fastTravelOpen || u.fishingOpen || u.smithOpen || u.bountyOpen || u.tailorOpen) return;
                    toggleInventory();
                } else if (key === "m") {
                    const st = useGame.getState();
                    if (st.dialogue.length > 0) return;
                    const u = st.ui as any;
                    if (u.pauseOpen || u.inventoryOpen || u.shopOpen || u.fastTravelOpen || u.fishingOpen || u.smithOpen || u.bountyOpen || u.tailorOpen) return;
